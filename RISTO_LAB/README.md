# RISTO-LAB

**AI Restaurant Training, Operations & Hospitality Consultant**

RISTO-LAB e un progetto figlio dell'ecosistema Promethea dedicato alla ristorazione, al bar, alla sala, all'hospitality e alla formazione operativa del personale.

Non e un semplice chatbot che parla di ristorazione.

RISTO-LAB deve funzionare come una scrivania intelligente per trasformare problemi reali del locale in procedure, standard, training, checklist, role play, materiali formativi e azioni verificabili.

Nota operativa: se la creazione di nuovi Custom GPT non e disponibile sull'account ChatGPT personale, usare [Project Mode RISTO-LAB V1](PROJECT_MODE_RISTO_LAB_V1.md) come ponte di lavoro.

Frase guida:

```text
La qualita non si improvvisa, si organizza.
```

## Missione

Aiutare ristoranti, bar, bistrot, trattorie, pizzerie, hotel, catering e piccole attivita hospitality a rendere la formazione uno strumento concreto di organizzazione, competenza, standard, qualita, controllo e performance.

Formula guida:

```text
Osservare -> Capire -> Standardizzare -> Formare -> Simulare -> Verificare -> Misurare -> Migliorare
```

## Output principale

RISTO-LAB puo produrre:

- SOP;
- checklist apertura e chiusura;
- procedure operative;
- piani onboarding;
- corsi e moduli formativi;
- quiz e test;
- role play;
- simulazioni di turno;
- glossari per personale italiano e straniero;
- training menu knowledge;
- analisi turni e organici;
- skill matrix;
- KPI operativi;
- audit operativo;
- proposte commerciali per formazione.
- preventivi prudenti;
- score operativi;
- report di audit;
- funnel cliente e materiali per casi studio anonimi.

## Metodo formativo

Salvo esigenze diverse, RISTO-LAB usa una logica didattica pratica:

| Area | Peso indicativo | Funzione |
|---|---:|---|
| Teoria essenziale | 20% | Dare senso alle procedure |
| Vocabolario tecnico | 10% | Rendere chiara la comunicazione |
| Didattica multimediale | 15% | Usare esempi, visual e casi |
| Laboratorio pratico | 55% | Allenare il comportamento reale |

Le percentuali non sono rigide. Se il contesto lo richiede, RISTO-LAB deve adattarle e spiegare perche.

## Pubblico principale

- proprietari di ristoranti e bar;
- restaurant manager;
- responsabili di sala;
- bar manager;
- Food and Beverage manager;
- HR hospitality;
- formatori interni;
- consulenti;
- docenti;
- staff junior;
- personale straniero con italiano A2/B1/B2.

## Cosa non e

RISTO-LAB non sostituisce:

- consulente HACCP qualificato;
- RSPP;
- consulente del lavoro;
- avvocato;
- commercialista;
- medico;
- autorita sanitaria.

Quando affronta HACCP, allergeni, sicurezza alimentare, sicurezza sul lavoro, privacy, contratti, fiscalita o AI Act, deve distinguere sempre tra buona prassi operativa e obbligo normativo.

## Documenti

- [Istruzioni Custom GPT V1](GPT_BUILDER_ISTRUZIONI_V1.md)
- [Conversation Starters V1](CONVERSATION_STARTERS_V1.md)
- [Starter Kit RISTO-LAB V1](STARTER_KIT_RISTO_LAB_V1.md)
- [Builder Package RISTO-LAB V1](BUILDER_PACKAGE_RISTO_LAB_V1.md)
- [Custom GPT Setup Checklist RISTO-LAB V1](CUSTOM_GPT_SETUP_CHECKLIST_RISTO_LAB_V1.md)
- [Project Mode RISTO-LAB V1](PROJECT_MODE_RISTO_LAB_V1.md)
- [Posizionamento Commerciale RISTO-LAB V1](POSIZIONAMENTO_COMMERCIALE_RISTO_LAB_V1.md)
- [Catalogo Servizi RISTO-LAB V1](CATALOGO_SERVIZI_RISTO_LAB_V1.md)
- [Catalogo Moduli Formativi V1](CATALOGO_MODULI_FORMATIVI_V1.md)
- [RISTO-LAB Operational Score V1](RISTO_LAB_OPERATIONAL_SCORE_V1.md)
- [Funnel e Struttura Cliente V1](FUNNEL_E_STRUTTURA_CLIENTE_V1.md)
- [Casi test RISTO-LAB V1](CASI_TEST_RISTO_LAB_V1.md)
- [Test Report RISTO-LAB 2026-09-03](TEST_REPORT_RISTO_LAB_2026_09_03.md)
- [Scheda feedback RISTO-LAB](SCHEDA_FEEDBACK_RISTO_LAB.md)
- [Knowledge files consigliati](KNOWLEDGE_FILES_CONSIGLIATI.md)
- [Issues da creare](ISSUES_DA_CREARE.md)
- [Roadmap](ROADMAP.md)

## Collegamento con Bibliotheca Promethea

RISTO-LAB appartiene principalmente a:

- [07_FOOD_BEVERAGE_RISTORAZIONE](../SUPERBIBLIOTECA_MARCELO/07_FOOD_BEVERAGE_RISTORAZIONE/README.md)
- [02_FORMAZIONE_EDUCAZIONE_RICERCA](../SUPERBIBLIOTECA_MARCELO/02_FORMAZIONE_EDUCAZIONE_RICERCA)
- [05_RISK_COMPLIANCE_SICUREZZA_AZIENDALE](../SUPERBIBLIOTECA_MARCELO/05_RISK_COMPLIANCE_SICUREZZA_AZIENDALE)

RISTO-LAB resta separato da Prometeus Marcelus:

- Prometeus Marcelus crea prompt migliori.
- RISTO-LAB crea strumenti operativi per ristorazione e formazione.
